...
export class Agent {
  constructor(id, playerId, name) {
    ...
    this.name = name;
    this.version = 1;
    // Random initial points between 1 and 5
    this.logicPoints = Math.floor(Math.random() * 5) + 1;
    this.dataStructurePoints = Math.floor(Math.random() * 5) + 1;
    this.abstractionPoints = Math.floor(Math.random() * 5) + 1;
    this.isBusy = false;
